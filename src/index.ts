export default {
  async fetch(request, env) {
    ...
    const { success } = await env.MY_D.prepare(`insert into data id value 1`).bind().run()
    ...
    let html = `<!DOCTYPE html>
      <head>
        <title> Takaaki: Request Cloudflare dump by GitHub Action </title>
      ...
      <body>
        ...
        <h1>Takaaki Workers: Request Cloudflare Edge dump by GitHub Action</h1>
